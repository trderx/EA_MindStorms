...
bool FilterStopOut( double profit_all, int MagicNumber)
{
    ...
    if (InpUseEquityStop) {
        ...
        if (profit_all < 0.0 && MathAbs(profit_all) > InpTotalEquityRisk / 100.0 * AccountEquity()) {
            if (InpCloseAllEquityLoss) { CloseThisSymbolAll(MagicNumber, 6); Print("Closed All to Stop Out"); }
            if (InpAlertPushEquityLoss) SendNotification("EquityLoss Alert " + (string)profit_all);
